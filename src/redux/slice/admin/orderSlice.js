...
export const getOrderDetail = createAsyncThunk(
  ...
  async (order_id) => {
    try {
      const res = await axios.get(URL_API + `order/${order_id}`, {
        withCredentials: true,
      });
      const data = await res.data;
      return data;
...
export const fetchAllOrder = createAsyncThunk(
  ...
  async () => {
    try {
      const res = await axios.get(URL_API + `order`);
      console.log("res", res.data);
      return res.data.getallOrder;
    } catch (error) {
      console.log(error);
    }
  }
  ...
  async (order_id) => {
    try {
      const res = await axios.put(
        URL_API + `order/confirm/${order_id}`,
        {
          withCredentials: true,
        }
      );
